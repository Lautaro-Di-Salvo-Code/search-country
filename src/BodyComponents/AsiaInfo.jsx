...
export const AsiaInfo = () => {
  ...
  const {dataCountries,search} = useContext(Contexto)
          ...
          let datos = dataCountries.filter( e => e.region === "Asia")
    ...
    md:grid-cols-4 
       ...
       overflow-hidden'>
        {resultado.map(e => (
           ...
           <Link key={e.name} to={`/Asia/${e.name}`} >{
            
       <div className='z-30 slide-fwd-center dark:bg-background-darksoft bg-background-lighthard rounded-[8px] hover:scale-110 ease-out duration-300 flex justify-center text-center w-[80%] h-auto  flex-col m-[2rem] pb-4   overflow-hidden  cursor-pointer ' key={e.numericCode} >
