...
export const AfricaSection = () => {
    ...
    let {dataCountries ,search } = useContext(Contexto)

    
            
            let datos = dataCountries.filter( e => e.region === "Africa")
...
if(!search ){
  ...
  resultado = datos.filter(i => i.name.toLowerCase().includes(search.toLowerCase()))
            ...
            // console.log(datos.map(e => e.flags.png))
    ...
    return (
        ...
        <Link key={e.name} to={`/Africa/${e.name}`} >{
